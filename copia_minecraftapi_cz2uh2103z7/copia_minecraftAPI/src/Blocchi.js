...
const Blocchi = () => {
	...
	const [toolSelezionato, setToolSelezionato] = useState('')
	 ...
	 <div>
		...
		<div className='container'>
			...
			{blocchi.map((elemento,indice) => (
				<div key={indice} className="blocco col-4">
					<img className='immagine-blocco' src={elemento.image} alt={elemento.name} style={{ width: '100px', height: '100px' }} onClick={()=>{dettaglio(elemento)}} />
					<div className='nome'>{elemento.name}</div>
				</div>
			))}
			</div>
		</div>
